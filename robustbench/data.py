...
from robustbench.loaders import CustomImageFolder, ImageNetLMDBDataset
...
def load_imagenetc(
    ...
    lmdb_data_path = str(data_folder_path).replace("ImageNet-C", "ImageNet-C-LMDB")
    ...
    if os.path.exists(f"{data_folder_path}/data.pt"):
        ...
        log.info(f"Loading from raw images (SLOW): {data_folder_path}")
