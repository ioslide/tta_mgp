...
def load_model_bayes(model_name : str,
               ...
               base_model : nn.Module = None) -> nn.Module:
    ...
    dataset_: BenchmarkDataset = BenchmarkDataset(dataset.split('_')[0])
    if norm is None:
    ...
    print("model_dir_:",model_dir_)
    print("Path(model_dir):",Path(model_dir))
    print("dataset_.value:",dataset_.value)
    print("threat_model_.value:",threat_model_.value)
    ...
    print(f"==>> swa_model_path:  {swa_model_path}")
    cov_model_path = model_dir_ / f'{cov_model_name}.pt'
    print(f"==>> cov_model_path:  {cov_model_path}")

    try:
        model = deepcopy(base_model)
        model_swa = deepcopy(base_model)
        model_cov = deepcopy(base_model)
        # if dataset_ == BenchmarkDataset.imagenet and 'Standard' in model_name: ## picking from saved ##
        #     return model.eval()
        ...
        # if dataset_.value in ['imagenet','imagenet_3dcc']:
    ...
    with torch.no_grad():
        ...
        for counter in iterator:
            ...
            y_curr = y[counter * batch_size:(counter + 1) *
                       batch_size].to(device)
            if eval_mode:
                output = model.forward(x_curr, y_curr, adapt=False)
            else:
...
def clean_accuracy1(model: nn.Module,
                   x: torch.Tensor,
                   y: torch.Tensor,
                   batch_size: int = 100,
                   is_enable_progress_bar: bool = False,
